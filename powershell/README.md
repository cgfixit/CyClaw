...
| `Invoke-CyClaw.ps1` | Starts the RAG gateway (`python gate.py`, so `gate.main()`'s bind guard and `api.tls` apply; host/port from `config.yaml`) from `~\.CyClaw\venv`. `-NoBrowser`, `-Repo`. If `CYCLAW_API_KEY` is unset, sources `%USERPROFILE%\.CyClaw\.env` then the repo `.env` (every Allow ACE must resolve to the current user SID, with at least one Allow ACE; shared, unresolvable, and empty ACLs are refused; refused HOME does not shadow repo). |
...
Credential Manager items are **not** deleted. The macOS uninstaller separately
offers an explicit `--remove-keychain` option.
