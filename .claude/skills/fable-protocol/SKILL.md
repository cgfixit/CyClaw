...
### 8.4-8.9 CyClaw facts, other projects, hardware, decisions log, API notes

Moved to `references/knowledge-base.md` 2026-09-11 (issue #1351: this was the
single largest chunk of what SessionStart injected in full on every session
start and every `/compact`). Read it when you need: the CyClaw flagship
facts-to-know-cold (six invariants, load-bearing numbers, common traps, git
workflow, recent trajectory — mostly a restatement of `CLAUDE.md`, which is
already in context separately), the other-projects list, hardware/environment
notes, the "decisions already made, don't re-litigate" table, GitHub-fetch
operational constraints, and Sonnet-5 API/model-tier provenance notes.
`SKILL.md` §8.1-8.3 above (identity, communication contract, "the pattern")
stay inline because they shape tone every turn; §8.4 onward is lookup material
for when a specific fact is actually needed.
...
— `fable-protocol` (discipline, §1-7) and a companion `fable-5.1-cc`
(knowledge, now §8 onward), merged once the split cost more than it bought.
Full history in `references/knowledge-base.md`'s closing section.
