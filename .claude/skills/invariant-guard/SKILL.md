...
| I2 | Topology = policy | Conditional edges exist ONLY at `route_by_score`, `guardrail_input`, `user_gate`, `pre_action_hook_grok`, and `pre_action_hook_claude`; `score_router` / `guardrail_router` / `user_gate_router` / `pre_action_hook_router` return exactly their documented target sets |
...
| I6 | Module isolation | AST imports both directions: `gate.py`/`gate_ops.py`/`gate_auth.py`/`gate_memory.py`/`graph.py`/`mcp_hybrid_server.py` never import `agentic`/`sync`/`guardrails`/`telegram`/`opentweet`, and no file under those packages imports the core set (`gate_ops.py`/`gate_auth.py`/`gate_memory.py` join the check because `gate.py` imports each of them directly, so anything they import is pulled transitively into `gate.py`'s process) |
| G1 | Telemetry kill | Three tiers, 14 files: `gate.py`'s `_TELEMETRY_KILL` assignment precedes its first heavy import (`graph`/`retrieval`/`llm`/`fastapi`/…); the five out-of-band package `__init__.py`s (agentic/guardrails/telegram/opentweet/sync) call `apply_telemetry_kill()` before EVERY other import; and the eight module-level appliers (mcp_hybrid_server, metrics, retrieval vector_store/indexer/clear_cache, guardrails/integration, utils gen_cert/authn_cli) call it before any third-party import |
