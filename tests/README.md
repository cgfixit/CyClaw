...
Pytest suite for this directory (`test_*.py` files are auto-collected
...
`--cov=` flags are passed. A new **source module** outside an already-covered
package needs a `--cov=` flag in `.github/workflows/ci.yml` AND an entry in
`[tool.coverage.run] source`; new test files are auto-discovered and need neither.
