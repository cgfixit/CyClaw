...
name: cyclaw-sandbox
description: >-
  CyClaw repository skill adapted from .claude/skills/CyClaw-Sandbox/SKILL.md. Use when working in CGFixIT/CyClaw and the user asks for this Claude skill workflow: clone origin/main to a clean local sandbox, install dependencies, run comprehensive config, server, tests, terminal, metrics, subsystem audits, produce the Local Sandbox Complete Audit report, and prepare a draft PR.
---

# Cyclaw Sandbox

Imported from `.claude/skills/CyClaw-Sandbox/SKILL.md` for Codex use in this repository. Do not edit the `.claude` source files when updating this Codex adapter; update this `.codex/skills` copy instead unless the user explicitly asks otherwise.
