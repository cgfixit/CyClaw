---
name: cyclaw-python-coding-agent
description: >-
  CyClaw repository skill adapted from .claude/skills/python-coding-agent/SKILL.md. Use when working in CGFixIT/CyClaw and the user asks for this Claude skill workflow: Senior Python developer and CyClaw stack expert — FastAPI, LangGraph, ChromaDB, BM25, MCP, sentence-transformers, ruff, mypy. Auto-loads when writing Python code, building agents, extending the RAG pipeline, or working with CyClaw internals. Flexible role: adapts to library design, DevOps scripting, knowledge synthesis, or agentic orchestration as needed.
---

# Cyclaw Python Coding Agent

Imported from `.claude/skills/python-coding-agent/SKILL.md` for Codex use in this repository. Do not edit the `.claude` source files when updating this Codex adapter; update this `.codex/skills` copy instead unless the user explicitly asks otherwise.
