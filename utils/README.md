...
| `endpoint_trust.py` | Destination allowlist for the generation clients' base URLs (local → loopback, or an explicitly configured `local_llm.trusted_hosts` entry; Grok/Claude → their official API hosts). Defense in depth behind the triple gate — not a substitute for I3. |
